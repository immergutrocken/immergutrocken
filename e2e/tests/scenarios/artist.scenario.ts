...
export class ArtistScenario {
  public static async createArtist(
    page: Page,
    prefix: string
  ): Promise<unknown> {
    const white1pxPngB64: string =
    ...
    const white1pxPngBuffer: Buffer = Buffer.from(
      Uint8Array.from(atob(white1pxPngB64), (c) => c.charCodeAt(0)).buffer
    );

    const deskPage: DeskPage = await DeskPage.openIn(page);
    await deskPage.contentMenuPanel.btnKuenstler.hover({
      force: true,
      timeout: 5000,
    });
    ...
    await expect(deskPage.artistPanel.languages.root).toBeVisible({
      timeout: 20000,
    });
    await expect(deskPage.artistPanel.languages.german.root).toBeVisible({
      timeout: 20000,
    });

    await deskPage.artistPanel.languages.german.inptTitle.fill(
      `${prefix} Name`
    );
    await deskPage.artistPanel.languages.german.inptSubtitle.fill(
      `${prefix} Untertitel`
    );

    await expect(deskPage.artistPanel.languages.german.banner.root).toBeVisible(
      { timeout: 2000 }
    );
    await deskPage.artistPanel.languages.german.banner.btnToggle.click();
    await deskPage.artistPanel.languages.german.banner.inptFile.setInputFiles({
      name: `${prefix}-1px-white.png`,
      mimeType: "image/png",
      buffer: white1pxPngBuffer,
    });
    await deskPage.artistPanel.languages.german.banner.inptCaption.fill(
      `${prefix} Image caption`
    );
    await deskPage.artistPanel.languages.german.banner.inptAlt.fill(
      `${prefix} Image alt`
    );
    await deskPage.artistPanel.languages.german.banner.inptCredits.fill(
      `${prefix} Image credits`
    );

    await deskPage.artistPanel.languages.german.textBoxContent.clickToActivateOverlay.click();
    await deskPage.artistPanel.languages.german.textBoxContent.textbox.fill(
      `${prefix} content`
    );

    await deskPage.artistPanel.languages.german.textBoxContent.btnAddYoutube.click();
    await deskPage.artistPanel.languages.german.textBoxContent.overlayYouTube.inptUrl.fill(
    ...
    await expect(
      deskPage.artistPanel.languages.german.textBoxContent.btnAddMedia
    ).toBeVisible({ timeout: 2000 });
    await deskPage.artistPanel.languages.german.textBoxContent.btnAddMedia.click();
    await expect(
      deskPage.artistPanel.languages.german.textBoxContent
        .btnAddMediaMenuEntryGalery
    ).toBeVisible({ timeout: 2000 });
    ...
    await deskPage.artistPanel.languages.german.textBoxContent.overlayBildergalerie.overlayEditBild.inptAltText.fill(
      `${prefix} alt`
    );
    await deskPage.artistPanel.languages.german.textBoxContent.overlayBildergalerie.overlayEditBild.inptCaption.fill(
      `${prefix} caption`
    );
    await deskPage.artistPanel.languages.german.textBoxContent.overlayBildergalerie.overlayEditBild.inptCredits.fill(
      `${prefix} credits`
    );
    ...
    await expect(deskPage.artistPanel.btnReviewChanges).toHaveText("just now", {
      timeout: 10000,
    });
    await expect(deskPage.artistPanel.btnPublish).toBeEnabled();
    await deskPage.artistPanel.btnPublish.click();

    const successToast: Toast = Toast.fromText(
      page,
  ...
  public static async deleteArtist(page: Page, prefix: string) {
    ...
    let successToast: Toast = Toast.fromText(
      page,
