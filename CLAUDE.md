...
- A newly published version has to age 5 days before it can get in. This is enforced **only in Renovate** — see the entry below on why the pnpm-side gate was removed:
...
- **Don't set `minimumReleaseAge` in `pnpm-workspace.yaml`.** It was tried (7200 = 5 days in minutes, pnpm counts minutes where Renovate takes a duration string) as the backstop Renovate can't provide — transitive deps and manual `pnpm add`/`pnpm update` — and it broke Renovate outright:
  - The setting roughly **doubles peak memory** of `pnpm install --lockfile-only --recursive`, the command Renovate runs to regenerate the lockfile. Measured on this repo: **2315 MB with, 1199 MB without** (49s vs 19s). Mend-hosted Renovate caps a job at 3.0 GB, so the job is OOM-killed part-way through the first branch and *no* dependency PRs get processed. `--network-concurrency=4` doesn't help (2388 MB) — the memory is retained version metadata, not in-flight requests
  - pnpm's resolver does **not** fall back to an older in-range version when the newest match is too fresh; it fails with `ERR_PNPM_NO_MATURE_MATCHING_VERSION` (pnpm#11203, open). So a fresh *transitive* release can break lockfile generation, not just an exact pin
  - It also blocks on any exact pin younger than the gate. `@argos-ci/playwright@7.4.2` was pinned before the gate landed, and every Renovate lockfile regeneration failed until it aged in. The resulting PRs carry a `package.json` change with **no lockfile**, so CI dies at `ERR_PNPM_OUTDATED_LOCKFILE` — the gate can't block `--frozen-lockfile` directly, but it blocks CI by leaving the two out of sync
  - Switching to Dependabot does not avoid this: it regenerates the pnpm lockfile the same way, and its `cooldown` conflicts with pnpm's setting in the same manner (dependabot/dependabot-core#13165, open). Self-hosting Renovate, where the memory limit is ours to set, is the option that would allow both layers
  - **pnpm 11 turns `minimumReleaseAge` on by default at 1440 minutes**, so this returns on the pnpm 11 upgrade unless it is explicitly set to `0`
